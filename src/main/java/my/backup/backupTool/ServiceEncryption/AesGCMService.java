...
public class AesGCMService extends BaseCalculationService implements ICopyService {

    private final BaseModel model;

    public AesGCMService(BaseModel model) {
        this.model = model;
    ...
    public void copyFileWithFileChannel(Path inputPath, Path outputPath, CryptoMODE cryptoMode) {
        ...
        if(file.length() > 2L * 1024L * 1024L * 1024L){
            String log = "";
            log += String.format("%-20s%s%n","Source:",inputPath);
            log += String.format("%-20s%s%n","FileIgnored:","File not copied");
            log += String.format("%-20s%s%n","DataSize:","AES_GCM is limited to 2 GiB file size in JAVA");
            log += String.format("%-20s%s%n","Solution:","Use AES_CTR Encryption for large files");
            LogFileWriterService.writeValidationLogFile(model.getUid(), LocalDateTime.now(), LogLEVEL.WARN, ValidationTYPE.SKIPPED,log);
            Platform.runLater(() -> {
                MessageService.createToast("AES_GCM limit reached", MessageTYPE.DANGER);
            });
            return;
        }

       Cipher cipher = initCipherEncryption(model.TransientProperties.getSecretKey(), model.TransientProperties.getGCMInitVector());
    ...
    public void decryptFileWithFileChannel(Path inputPath, Path outputPath) {
        Cipher cipher = initCipherDecryption(model.TransientProperties.getSecretKey(), model.TransientProperties.getGCMInitVector());
    ...
    public void generateNewSecretKeyAndIVectorAndSetThem() {
        ...
        model.TransientProperties.setGCMInitVector(iv);
